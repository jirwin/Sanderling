...
+ In your local chat window go to settings and clear all contents.
...
//    assuming that the windowstack will have local first and that when it says anything other than local (eg. local [4]) then the number in brackets is the count of people in local
//    any number is bad as it never says [1] which would be for just one self
//bool hostileOrNeutralsInLocal => "LOCAL" != Sanderling.MemoryMeasurementParsed?.Value?.WindowStack?.FirstOrDefault()?.LabelText?.FirstOrDefault()?.Text.Trim().ToUpper();
//    Above only works in highsec. In lowsec you don't get counts.
//    Workaround is to clear local window. Then there should be exactly 3 labels.
		// Local
		// +
		// Your character name
//    If there are more than those 3 then you have a  problem - someone else is with you.
bool hostileOrNeutralsInLocal => 3 != Sanderling.MemoryMeasurementParsed?.Value?.WindowStack?.FirstOrDefault()?.LabelText?.ToList().Count;
